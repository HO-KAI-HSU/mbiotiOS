...
struct IndexView: View {
    ...
    func Refresh() {
        ...
        self.userSetting = UserSeting(url: url, userKey:key, uuid : "", isAuth: false)
        let uid = GetUuid()
        let isAuth = CheckIsAuth(uid: uid, url : url, key : key)
        self.userSetting = UserSeting(url: url, userKey:key, uuid : uid, isAuth: isAuth)
    }
}
...
#Preview {
    ...
    let url = userDefault.string(forKey: "url_preference") ?? ""
    let key = userDefault.string(forKey: "key_preference") ?? ""
    return IndexView(userSetting: .constant(UserSeting(url: url, userKey:key, uuid : "", isAuth: false)),
